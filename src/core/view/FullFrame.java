...
public class FullFrame {
	...
	public FullFrame(OrgsecModel model) {
	...
	private JScrollPane getServerPanel(NetworkModel model, OutputStream out) {
		...
		for (int i = 0; i < servers.length; i++) {
			...
			DefaultMutableTreeNode root = new DefaultMutableTreeNode(servers[i] + "-" + model.getUnitCount(servers[i]));
			String[] profiles = model.getServerProfiles(servers[i]);
	...
	private JScrollPane getDevicePanel(NetworkModel model) {
		...
		for (int i = 0; i < devices.length; i++) {
			...
			JLabel label = new JLabel(devices[i]);
			panel.add(label, g);
		}
